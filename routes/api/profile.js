...
router.get('/', async (req, res) => {
  try {
    // Add avatar and name from other table
...
router.delete('/experience/:exp_id', auth, async (req, res) => {
  try {
    ...
    // if (removeIndex === -1) {
    //   return res.status(404).json({ msg: "That experience ID does not exist" })
    // }
...
router.get('/github/:username', (req, res) => {
  try {
    ...
    const options = {
      uri: `https://api.github.com/users/${req.params.username}/repos?per_page=5&sort=created:ascc&client_id=${config.get('githubClientId')}&client_secret=${config.get('githubSecret')}`,
      method: 'GET',
